		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+90.0) / FULL_CIRCLE) * PI * 2.0;
...
void render()
{
	...
	ggprint8b(&r, 16, 0x00ffff00, "n asteroids destroyed: %i", g.aDestroyed);
	//
	//-------------
	//Draw the ship
	...
	//Draw the asteroids
	{
		...
		while (a) {
			...
			glEnd();
			glPopMatrix();
			glColor3f((rand())*1.0f, (rand())*1.0f, (rand())*1.0f);
			glBegin(GL_POINTS);
